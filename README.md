...
Moosic은 음원 스트리밍 서비스입니다.

관리자, 유저, 배급사 계정 관리 - 회원가입, 회원탈퇴, 로그인, 로그아웃, 프로필수정

서비스 구독 - 결제정보등록, 결제정보수정, 결제정보조회, 구독취소 (결제와 환불은 기능구현 X)

음원서비스 - 앨범 등록, 음원 등록, 음원재생, 재생이력등록, 재생이력조회, 재생횟수조회
...
다른 한편 오늘 막 취업을 한 최세호 씨는 자신의 관리자 계정을 얻기 위해 **관리자 계정 회원가입**을 진행하였고, 로그인을 하였는데 막 배급사 등록을 요청한 김세리 씨의 요청을 수락하였다(**급하게 오늘 떠올라서 가정으로 하겠습니다.ㅠㅠ**).
